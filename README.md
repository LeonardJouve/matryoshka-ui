...
| `Percent(percent float64)` | Size as a percentage (`0..100`) of the parent's inner (content-box) size on that axis |

A note on `Percent`: because a percentage depends on the parent's resolved size, it is resolved top-down during the `flex` pass rather than during the bottom-up `sizing` pass. This means a `Percent` child contributes nothing to a `Fit`-sized parent's measured size and would collapse to zero there. Use `Percent` only inside parents whose size is determined independently — `Fixed`, `Grow`, or another already-resolved `Percent`. Percentages are not clamped against siblings, so three `Percent(50)` children on the same axis will overflow, exactly as in CSS.
...
## Component library and example screens

The `components` package and the example screens that ship alongside it (login, settings, profile, chat, pricing, music player, inbox and a dashboard demo) were **generated by Claude (Anthropic)**. They were produced collaboratively in a session where the library's public API was shared and trusted as expressive enough for an AI to build real UIs against — Claude composed the components and screens purely from the documented DSL (`Div`/`Text`/`Image`, the style modifiers, and the sizing model including `Percent`).

Each generated file was type-checked with `go build` and `go vet` against the actual source, and every screen was run through the full layout pass (`sizing` → `flex` → `position`) headless to confirm it produces geometry without panicking. They were **not** rendered to actual pixels in that environment (the raylib renderer needs native C/OpenGL libraries), so treat the visual spacing — especially hard-coded fixed widths — as a starting point to eyeball and tune locally with `go run .`.

Two caveats carried over from the engine itself: the generated "buttons", "toggles", "tabs", and "checkboxes" are **visual only** (there is no input/event layer, so their states are styling you set at build time, not interactive behavior), and any component using `Percent` (e.g. the progress bar) requires the `Percent` sizing support to be present in your tree.
